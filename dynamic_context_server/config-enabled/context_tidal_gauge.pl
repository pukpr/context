...
:- context:register(context_tidal_gauge:navigate).
:- context:register(context_tidal_gauge:plot).

% biennial(1.9, -3.6, 4.4, -0.8, 1.3, 2.7, -0.0).
biennial(1.9, 0.9, 0.2, 3.9, 0.2, 2.7, 0.6).

bie_sin_period(0.0,N,M,_,0) :- N =< M.
bie_sin_period(Period,N,M,X,Y) :-  biennial(A,B,C,D,E,F,G),
    N =< M, test_period(N,Period,P), Y is (G+A*cos(pi/12*X+B)+C*cos(3*pi/12*X+D)+E*cos(5*pi/12*X+F))*sin(X*(2*pi/P)).
bie_sin_period(_,N,M,_,0.0) :- N > M.

bie_cos_period(0.0,N,M,_,0) :- N =< M.
bie_cos_period(Period,N,M,X,Y) :-   biennial(A,B,C,D,E,F,G),
    N =< M, test_period(N,Period,P), Y is (G+A*cos(pi/12*X+B)+C*cos(3*pi/12*X+D)+E*cos(5*pi/12*X+F))*cos(X*(2*pi/P)).
bie_cos_period(_,N,M,_,0.0) :- N > M.
...
navigate(Request) :-
   ...
   con_text:collect_options(context_tidal_gauge:temp_data, DataSet),
...
temperature_series(Second, Window, Triple, DataSet, T, Fit, Offset) :-
   ...
   (  Second ->
      TDD mapdot TT4 - 0.03 .* TT5,  % 0.03 TDD mapdot TT4 - 0.036 .* TT5,
      T2 mapdot TDD + 0.001 .* TT8,  % 0.001 T2 mapdot TDD + 0.0006 .* TT8,
      T mapdot T2 + 1.1 .* TTT  % 1.1
      % T = TT4
   ;
      TDD mapdot TT4 - 0*0.011 .* TT5,
      T mapdot Scaled0 - Fit*0.01 .* TDD
      % T = Scaled0
   ),
   Offset mapdot 0 .* TT.
...
select_period(true,_,ENSO,R,_Add) :- R is ENSO*12.
...
*/
    ...
    select_period(Long, YearThird, 2.33333, PeriodE,1*Add), % 0.67135 YearThird 0.454 0.542304705
    % PeriodF is Add*Trop1*12, % 0.8245 Trop1 18.6 2*pi/1.266993*12, %  27.67 weak
    select_period(Long, Trop1, Trop1, PeriodF,1*Add), %3.53
    ...
    select_period(Long, 1.81, 2.0427, PeriodH,1*Add), % 1.528 1.81 0.528 0.2428 5.695

    Sin mapdot qbo_sin_period(Period,1,WL) ~> Months,
    Cos mapdot qbo_cos_period(Period,1,WL) ~> Months,
    Sin2 mapdot bie_sin_period(Period2,2,WL) ~> Months,
    Cos2 mapdot bie_cos_period(Period2,2,WL) ~> Months,
    Sin3 mapdot bie_sin_period(Period3,3,WL) ~> Months,
    Cos3 mapdot bie_cos_period(Period3,3,WL) ~> Months,
    Sin4 mapdot bie_sin_period(Period4,4,WL) ~> Months,
    Cos4 mapdot bie_cos_period(Period4,4,WL) ~> Months,
    Sin5 mapdot bie_sin_period(Period5,5,WL) ~> Months,
    Cos5 mapdot bie_cos_period(Period5,5,WL) ~> Months,
    Sin6 mapdot bie_sin_period(Period6,6,WL) ~> Months,
    Cos6 mapdot bie_cos_period(Period6,6,WL) ~> Months,
    Sin7 mapdot bie_sin_period(Period7,7,WL) ~> Months,
    Cos7 mapdot bie_cos_period(Period7,7,WL) ~> Months,
    Sin8 mapdot qbo_sin_period(Period8,8 ,WL) ~> Months,
    Cos8 mapdot qbo_cos_period(Period8,8 ,WL) ~> Months,
    Sin9 mapdot qbo_sin_period(Period9,9 ,WL) ~> Months,
    Cos9 mapdot qbo_cos_period(Period9,9 ,WL) ~> Months,
    SinA mapdot bie_sin_period(PeriodA,10,WL) ~> Months,
    CosA mapdot bie_cos_period(PeriodA,10,WL) ~> Months,
    SinB mapdot qbo_sin_period(PeriodB,11 ,WL) ~> Months,
    CosB mapdot qbo_cos_period(PeriodB,11 ,WL) ~> Months,
    SinC mapdot qbo_sin_period(PeriodC,12 ,WL) ~> Months,
    CosC mapdot qbo_cos_period(PeriodC,12 ,WL) ~> Months,
    SinD mapdot qbo_sin_period(PeriodD,13,WL) ~> Months,
    CosD mapdot qbo_cos_period(PeriodD,13,WL) ~> Months,
    SinE mapdot qbo_sin_period(PeriodE,14,WL) ~> Months,
    CosE mapdot qbo_cos_period(PeriodE,14,WL) ~> Months,
    SinF mapdot qbo_sin_period(PeriodF,15,WL) ~> Months,
    CosF mapdot qbo_cos_period(PeriodF,15 ,WL) ~> Months,
    Sin0 mapdot qbo_sin_period(PeriodH,16,WL) ~> Months,
    Cos0 mapdot qbo_cos_period(PeriodH,16,WL) ~> Months,
    ...
    BN4 is  Beat_Node/8,
    select_period(Long, 0.836,0.86503, CWobble,1*Add),  % ThreeSyn 0.53 0.56372 0.58 0.531
    ...
    select_period(Long, YearHalf, 0.5, K1,0*Add), % YearHalf 0.08721149897330595
    % select_period(Long, YearHalf, Evec, K1), %3.64
    % select_period(Long, YearHalf, 100.0, K1),

    TSI_F mapdot qbo_sin_period(K1,0,WL) ~> Months,
    AAM mapdot qbo_cos_period(K1,0,WL) ~> Months,
