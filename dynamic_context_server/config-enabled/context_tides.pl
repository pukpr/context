...
:- context:register(context_tides:navigate).
:- context:register(context_tides:plot).
...
yearly_tides(X,Y) :- Y is 1.0 - 0.15*cos(2*pi*X/12-2.6).

%biamp(X,Amp) :- Amp is (1-0.2*cos(2*pi/(10.1*12)*X - 0.7)).
%bimod(X,Mod) :- Mod is 0.7*cos(2*pi/(17.9*12)*X + 0.9).

d_per(P) :- P is 2.0.
d_mod1(X,Amp) :- Amp is 2.6*cos(2*pi/12/51.0*X - 1.5).
d_mod2(X,Amp) :- Amp is 0.1*cos(2*pi/12/6.2*X + 4.0).
% biamp(_,1.0).
% bimod(_,0.0).
% sin_biennial_period(X,Y) :- X > 76*12, bimod(X,M), biamp(X,A), Y is A*sin(2*pi*X/24+M), !.
%
sin_bi_period(X,Y) :-  d_per(P), d_mod1(X,M1), d_mod2(X,M2), Y is -1.0*sin(2*pi*X/12/P+M1+M2).

%  cos_biennial_period(X,Y) :- X > 76*12, bimod(X,M), biamp(X,A), Y is A*cos(2*pi*X/24+M), !.
%
cos_bi_period(X,Y) :-  d_per(P), d_mod1(X,M1), d_mod2(X,M2), Y is -1.0*cos(2*pi*X/12/P+M1+M2).
...
mathieu_tides(X,Y) :- Y is -0.7*(1.0-0*0.4*cos(2*pi*X/2.33-2.4)).
...
navigate(Request) :-
   ...
   con_text:collect_options(context_tides:temp_data, DataSet),
...
mathieu_tides(Z,X,Y) :- Y is 1.0+Z*0.4*cos(2*pi*X/9.08-4.1).
...
temperature_series(Second, Window, Triple, DataSet, T, Fit, Offset) :-
   ...
   (   Flip == 1.0 ->
       Scal is 0.1;
       Scal is 1.05  % 1.05 0.8
   ),
   % Scal is -0.35-0.2*Flip,
   Omega2 mapdot Scal*mathieu_tides(0) ~> Year, % 0.8
   Scaled0 mapdot Omega2 * TTT,
   Front range 1*[1,1214],  % 1188 1215
   Middle range Flip*[1215,1393],  % 1386
   Back range 1*[1394,L],  % 1387
...
*/
    ...
    select_period(Long, 0.0, 4.425, Period,1*Add), %2.9
    % Period4 is Sc*Drac2*12, % 0.4129 1.73 3.53 Add*Hale/1 *12 *Q,
    select_period(Long, 3.41, Drac, Period2,1*Add),
    % Period2 is Sc*Trop*12, %2*pi/2.3157*12, %  27.32 tropical
    select_period(Long, 1.896, Trop, Period3,1*Add),
    % Period3 is Sc*Drac1*12, % 0.701 4.15 3.0243*12, %  27.093 also  31.81 evection

    Saros is 18.6,
    BiDracPlus is 1/(1/2+1/Saros),
    BiDracMinus is 1/(1/2-1/Saros),

    select_period(Long, BiDracMinus, 19.2, Period4,1*Add), % 18.613
    % Period5 is Sc*Drac3*12, % 0.317 18.6 0.717 2.505 1.93 1.583 0*7.944*12, % 7.944 2.54  Venus 9.315*12*Q,  %
    select_period(Long, 0.0, 4.04, Period5,0*Add), % 4.06

    % Period6 is Sc*Drac4*12, % 0.222 1.583 9.3 2*pi/1.6055*12, %  27.55 weak
    select_period(Long, BiDracPlus, 3.5743, Period6,1*Add), % 6.65
    % Period7 is Sc*Drac5*12,  % 0.1825 1.35777 18.6 2*pi/1.266993*12, %  27.67 weak
    select_period(Long, 2.0, 6.5, Period7,1*Add), % 9.3067

    % Period8 is -Anom*12, % 1.344  2*pi/2.475*12 27.555 anomalistic
    select_period(Long, 0.0, Anom1, Period8,1*Add), %3.64
    Period9 is 0*Evec*12,       % 2.094 14.77 spin-orbit

    %PeriodA is 2.975*12, % Hale/6 *12 *Q,
    select_period(Long, AnomHalf, 42.66, PeriodA,1*Add), % 42.66, 8.8512
    % PeriodC is MfP*12, % 2.528 Sc*8.848*12/2, % 1.7579*12, %  1305.48  somewhat weak

    select_period(Long, MfP, MfP, PeriodB,1*Add),
    select_period(Long, 0.0, 2.898, PeriodC,1*Add), % 4.13
    ...
    select_period(Long, 2.337, 9.07, PeriodF,1*Add), %3.53
    ...
    select_period(Long, 0.0, 5.65, PeriodH,0*Add), % 5.695
    ...
    select_period(Long, 3.06, BA4, Evection,1*Add),
    ...
    % BN4 is  Beat_Node/8 + 0.0066, %%%%% Look at the plus SIGN !!!!
    BN4 is 2.01, %%%%% Look at the plus SIGN !!!!
    select_period(Long, BN4, BN4, CWobble,1*Add),
    V1 mapdot sin_bi_period ~> Months,
    LOD_F mapdot cos_bi_period ~> Months,
    ...
    select_period(Long, 0.0, 2.274, K1,0*Add), % 0.08721149897330595
    ...
    % TSI_F mapdot sin_bi_period ~> Months,
    % AAM mapdot cos_bi_period ~> Months,
    ...
    T_R_mod mapdot Boost*yearly_tides ~> Months,
