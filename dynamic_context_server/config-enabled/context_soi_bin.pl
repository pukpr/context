...
:- context:register(context_soi_bin:navigate).
:- context:register(context_soi_bin:plot).

% amp_phase(1.1,1.9).
% amp_phase(0.0,1.9).
amp_phase(0.0,2.3).  % 2.1 1.92
%mod_phase(0.65,-0.12,-1.5).
% mod_phase(0.0,-0.0,0.0).
mod_phase(0.2,-0.0,2.3).

bin_sin_period(0.0,N,M,_,0) :- N =< M.
bin_sin_period(Period,N,M,X,Y) :- amp_phase(A,P), mod_phase(B,Mod,S),
	  % Ph = 0.0, % Ph is B*sin(2*pi/Mod*X+S),
	 N =< M, Y is sin(2*pi*X/Period)*(A+1*sin(2*pi*X/24+P)+B*sin(2*pi*X/12+S)+Mod*sin(2*pi*X/12) ).
bin_sin_period(_,N,M,_,0.0) :- N > M.

bin_cos_period(0.0,N,M,_,0) :- N =< M.
bin_cos_period(Period,N,M,X,Y) :- amp_phase(A,P), mod_phase(B,Mod,S),
	  % Ph = 0.0, % Ph is B*sin(2*pi/Mod*X+S),
	 N =< M, Y is cos(2*pi*X/Period)*(A+1*sin(2*pi*X/24+P)+B*sin(2*pi*X/12+S)+Mod*sin(2*pi*X/12) ).
bin_cos_period(_,N,M,_,0.0) :- N > M.

%mathieu_bin_modul(Z,X,Y) :- Y is 1.0+Z*cos(2*pi*X/2-0.1)-Z*0.4*cos(2*pi*X/4-1.4).
%
% mathieu_bin_modul(Z,X,Y) :- Y is
% 1.0+5.5*cos(2*pi*X/2+0.3)-0*0.2*cos(2*pi*X/6+1.6).
mathieu_bin_modul(A,Z,X,Y) :- Y is A+Z*1.2*cos(2*pi*X/2+0.15)+Z*0.4*cos(2*pi*X/1)-Z*0.0*cos(2*pi*X/4-1.3).
% mathieu_bin_modul(A,Z,X,Y) :- Y is A+Z*cos(2*pi*X/2+0.2)+Z*0.37*cos(2*pi*X/1)+Z*0*0.18*cos(2*pi*X/3+1.9).


amp_bin_phase(1.2,3.6).

quad_sin_period(0.0,N,M,_,0) :- N =< M.
quad_sin_period(Period,N,M,X,Y) :- amp_bin_phase(A,P),
	 N =< M,  Y is sin(2*pi*X/Period)*(0.0+A*sin(2*pi*X/48+P)).
quad_sin_period(_,N,M,_,0.0) :- N > M.

quad_cos_period(0.0,N,M,_,0) :- N =< M.
quad_cos_period(Period,N,M,X,Y) :- amp_bin_phase(A,P),
	N =< M, Y is cos(2*pi*X/Period)*(0.0+A*sin(2*pi*X/48+P)).
quad_cos_period(_,N,M,_,0.0) :- N > M.


bin_seasonal_mod(X,Y) :- Y is 1.0 + 0.2*cos(pi*X/12-0.6)^2+0.0*cos(pi*X/6-0.9)^2.
% bin_seasonal_mod(X,Y) :- Y is 1.0 + 0.9*cos(pi*X/12+0.2)^2+
% 0.0*cos(2*pi*X/4+0.4).
...
% http://localhost/context_soi_bin/plot?startYear=1920&fitYear=2013
...
navigate(Request) :-
   ...
   con_text:collect_options(context_soi_bin:temp_data, DataSet),
			  ...
			  small(\multi_columns([
					...
					\(con_text:input_cells([[days,365.242181,6],
					 ...
					 '@import url("/html/css/context.css")'),
					  ...
					  input([type('text'), size(2), name('wave'), value('10')]),
...
get_fit(StartDate, EndDate, [Temperature, CO2, SOI,  Volc,   LOD,  TSI, AAM,   Arctic, NAO, Sin, Cos,
   ...
   B is (EndDate - 1866+300*0)*12,
   O is (StartDate - 1866+300*0)*12 + 1,
   % O = 1, % 840,
...
triple_filter(In, Out) :-
    % median_filter(In, In0),
    % A window In*11,  %7 9 11 13
    B window In*5,   %5 7 7 9
...
temperature_series(Second, Window, Triple, DataSet, T, Fit, Offset) :-
   ...
   get_fit(Fit, Flip),
   get_years_from_1866(TTT, Year, _),
   ...
   % Omega2 mapdot 1.0*mathieu_bin_modul(2.0, 3.2) ~> Year, % 3.2 0.8
   Omega2 mapdot 2.9*mathieu_bin_modul(1.3, 1.5) ~> Year, % 2.7 3.2 0.8
   T_Om mapdot TSI * TTT,
   Scaled0 mapdot Omega2 * TTT  - 0*0.6 .* T_Om,
   Bottom is (300*0+1)*12, BottomPlus is Bottom+1,
   Top is (300*0+14)*12, TopPlus is Top+1,
   Lower is 1216+168+300*0*12, LowerPlus is Lower+1,
   Upper is 1398+168+300*0*12, UpperPlus is Upper+1,
   Base range 1*[1,Bottom],
   Initial range Flip*[BottomPlus,Top],
   Front range 1*[TopPlus,Lower],  % 1188 1215
   Middle range Flip*[LowerPlus,Upper],  % 1384 1395 1386
   Back range 1*[UpperPlus,L],  % 1385 1396 1387
   Clip cat [Base,Initial,Front,Middle,Back],
       % Offset invert Clip,
   ...
   % Scaled mapdot Scaled0 - 0.24 .* TT4,  % - 0.15 .* TT
   ...
   (  Second ->
      ...
      % T = TT
   ;
      % Omega2 mapdot Scal*mathieu_bin_modul(0.0) ~> Year, % 0.8
      % TT_E mapdot Scaled0 - 0.06 .* TT ,
      TT_E mapdot Scaled0 - 0.3 .* TT4 - 0.00000 .* TT8 ,
      T mapdot TT_E * Clip
...
get_years_from_1866(T, Years, Zeros) :-
    length(T, L),
    H range [1, L]/1,
    Y mapdot H ./ 12.0,
    Start is 1866-300*0,
    Years mapdot Start .+ Y,
    Zeros mapdot 0 .* H.
...
plot(Request) :-
    ...
    get_years_from_1866(T, Year, Zeros),
...
*/
    ...
    select_period(Long, Anomalistic, 1.939393939, Period3,1*Add), % 6.65
    % Period3 is Sc*Drac1*12, % 0.701 4.15 3.0243*12, %  27.093 also  31.81 evection
    HalfTrop is Tropical/2.0,
    Drac_Div3 is Draconic/6,
    select_period(Long, Synodical, 2.081300813, Period4,1*Add), % 1.757 4.09
    select_period(Long, HalfTrop, 2.0, Period5,1*Add), % 1.53 9.3067
    ...
    */
    ...
    select_period(Long, LL_Plus, 1.53,      Period7,1*Add),
    ...
    */
    ...
    select_period(Long, HalfNode, 9.07, PeriodF,1*Add), % 6.42, 1.591 3.53
    ...
    select_period(Long, HalfSBMinus, 5.65, PeriodG,0*Add), % 5.65 1.438 3.82
    select_period(Long, MfPrime, 5.65, PeriodH,0*Add), % 1.0 3.63 5.695

    Sin mapdot bin_sin_period(Period,1,WL) ~> Months,
    Cos mapdot bin_cos_period(Period,1,WL) ~> Months,
    Sin2 mapdot bin_sin_period(Period2,2,WL) ~> Months,
    Cos2 mapdot bin_cos_period(Period2,2,WL) ~> Months,
    Sin3 mapdot mod_sin_period(Period3,3,WL) ~> Months,
    Cos3 mapdot mod_cos_period(Period3,3,WL) ~> Months,
    Sin4 mapdot bin_sin_period(Period4,4,WL) ~> Months,
    Cos4 mapdot bin_cos_period(Period4,4,WL) ~> Months,
    ...
    Sin7 mapdot bin_sin_period(Period7,7,WL) ~> Months,
    Cos7 mapdot bin_cos_period(Period7,7,WL) ~> Months,
    ...
    SinA mapdot bin_sin_period(PeriodA,10,WL) ~> Months,
    CosA mapdot bin_cos_period(PeriodA,10,WL) ~> Months,
    ...
    SinD mapdot bin_sin_period(PeriodD,13,WL) ~> Months,
    CosD mapdot bin_cos_period(PeriodD,13,WL) ~> Months,
    SinE mapdot bin_sin_period(PeriodE,14,WL) ~> Months,
    CosE mapdot bin_cos_period(PeriodE,14,WL) ~> Months,
    SinF mapdot bin_sin_period(PeriodF,15,WL) ~> Months,
    CosF mapdot bin_cos_period(PeriodF,15 ,WL) ~> Months,
    SinG mapdot mod_sin_period(PeriodG,16,WL) ~> Months,
    CosG mapdot mod_cos_period(PeriodG,16,WL) ~> Months,
    Sin0 mapdot mod_sin_period(PeriodH,17,WL) ~> Months,
    Cos0 mapdot mod_cos_period(PeriodH,17,WL) ~> Months,
    ...
    LogCO2 mapdot bin_sin_period(Evection,17,WL) ~> Months,
    S2 mapdot bin_cos_period(Evection,17,WL) ~> Months,
    ...
    V1 mapdot bin_sin_period(CWobble,0,WL) ~> Months,
    LOD_F mapdot bin_cos_period(CWobble,0,WL) ~> Months,
