...
:- context:register(context_soi:navigate).
:- context:register(context_soi:plot).
...
navigate(Request) :-
   ...
   con_text:collect_options(context_soi:temp_data, DataSet),
			  ...
			  small(\multi_columns([
					...
					\(con_text:input_cells([[days,365.242181,6],
					 ...
					 '@import url("/html/css/context.css")'),
					  ...
					  input([type('text'), size(2), name('wave'), value('19')]),
					  \(con_text:check_box(fit_res, 1, 'phase flip')),
					  \(con_text:check_box(high_res, 1, 'integrated', checked))
...
% mathieu_modul(Z,X,Y) :- Y is 1.0+Z*0.4*cos(2*pi*X/9.08-4.1).
mathieu_modul(Z,X,Y) :- Y is 1.0+Z*0.55*cos(2*pi*X/2-0.6).
...
temperature_series(Second, Window, Triple, DataSet, T, Fit, Offset) :-
   ...
   (   Flip == 1.0 ->
       Scal is 1.03;
       Scal is -18.0  % 1.05 0.8
   ),
   % Scal is -0.35-0.2*Flip,
   Omega2 mapdot Scal*mathieu_modul(0) ~> Year, % 0.8
   Scaled0 mapdot Omega2 * TTT,
   Front range 1*[1,1215],  % 1188 1215
   Middle range Flip*[1216,1395],  % 1384 1395 1386
   Back range 1*[1396,L],  % 1385 1396 1387
   ...
   Scaled mapdot Scaled0 + Offset .* TT,
...
*/
    ...
    T_R_mod mapdot Boost*yearly_qmodul ~> Months,
    get_fit(Second, De),
