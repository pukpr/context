:- module(context_sea_level_height, [
			       mathieu_modulater/3,
			       yearly_qmod/2
	  ]).
...
:- context:register(context_sea_level_height:navigate).
:- context:register(context_sea_level_height:plot).
...
temp_data('Tides Processed', sydney_processed).
...
dataset(Num, L) :-
	...
	context_sea_level_height_singapore_data:qbo_from_1987(N, L0),
	qbo_padding_1987(L0, L1),
	qbo_offset(N, Offset),
	L mapdot Offset .+ L1.

dataset(qbo_10, L) :-
        context_sea_level_height_data:qbo_10(L0),
	qbo_padding(L0, L1),
	L mapdot 75 .+ L1.
dataset(qbo_15, L) :-
        context_sea_level_height_data:qbo_15(L0),
	qbo_padding(L0, L1),
	L mapdot 85 .+ L1.
dataset(qbo_20, L) :-
        context_sea_level_height_data:qbo_20(L0),
	qbo_padding(L0, L1),
	L mapdot 85 .+ L1.
dataset(qbo_30, L) :-
        context_sea_level_height_data:qbo_30(L0),
	qbo_padding(L0, L1),
	L mapdot 105 .+ L1.
dataset(qbo_40, L) :-
        context_sea_level_height_data:qbo_40(L0),
	qbo_padding(L0, L1),
	L mapdot 85 .+ L1.
dataset(qbo_50, L) :-
        context_sea_level_height_data:qbo_50(L0),
	qbo_padding(L0, L1),
	L mapdot 68 .+ L1.
dataset(qbo_70, L) :-
        context_sea_level_height_data:qbo_70(L0),
	qbo_padding(L0, L1),
	L mapdot 30 .+ L1.
dataset(qbo_200, L) :-
        context_sea_level_height_data:qbo_200(L0),
...
navigate(Request) :-
   ...
   con_text:collect_options(context_sea_level_height:temp_data, DataSet),
...
temperature_series(Second, Window, Triple, DataSet, T, Fit, Offset) :-
   ...
   (  Second ->
      TDD mapdot TT4 - 0.08 .* TT5,  % TDD mapdot TT4 - 0.036 .* TT5,
      T2 mapdot TDD - 21.8 .* TT1,  % T2 mapdot TDD + 0.0006 .* TT8,
      T mapdot T2 + 0.52 .* TTT   % 0.62
      % T = TT4
   ;
...
*/
    ...
    select_period(Long, 18.6, 18.6, PeriodE,0*Add), % 0.7638 YearThird 0.454 0.542304705
    (	Fit = 1 ->
    ...
    select_period(Long, 1.654, 0.666666, PeriodH,0*Add), % 1.654 Trop2 1.81 0.528 0.2428 5.695
    ...
    BN4 is  Beat_Node/8,
    select_period(Long, 22.1,6.01, CWobble,1*Add),  % Anom3 0.53 0.56372 0.58 0.531
    ...
    select_period(Long, 0.5, YearHalf, K1,0*Add), % 0.08721149897330595
